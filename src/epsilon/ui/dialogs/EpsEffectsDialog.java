...
public class EpsEffectsDialog extends BaseDialog {
    ...
    public EpsEffectsDialog(){
        this(EpsLogicFx.entries());
    }

    public static EpsEffectsDialog withAllEffects(){
        return new EpsEffectsDialog(Seq.select(Fx.class.getFields(), f -> f.getType() == Effect.class).map(f -> new EffectEntry(Reflect.get(f)).name(f.getName())));
    }
