...
public class EpsShieldWall extends Wall{
    ...
    public class EpsShieldWallBuild extends WallBuild{
        ...
        public void draw(){
            ...
            if(enabled){
                if (this.shieldRadius > 0.0F) {
                    float radiusN = this.shieldRadius * 8.0F * (float)EpsShieldWall.this.size / 2.0F * radius;
                    Draw.z(125.0F);
                    ...
                    Draw.reset();
                    Drawf.additive(EpsShieldWall.this.glowRegion, EpsShieldWall.this.glowColor, (1.0F - EpsShieldWall.this.glowMag + Mathf.absin(EpsShieldWall.this.glowScl, EpsShieldWall.this.glowMag)) * this.shieldRadius, this.x, this.y, 0.0F, 31.0F);
                }
            }
        }
        @Override
        public void updateTile(){
            if(enabled){
                if (this.breakTimer > 0.0F) {
                    ...
                    this.shield = Mathf.clamp(this.shield + EpsShieldWall.this.regenSpeed * this.edelta(), 0.0F, EpsShieldWall.this.shieldHealth);
                }
        ...
        public void damage(float damage) {
            if(enabled){
                ...
                if (this.shield <= 1.0E-5F && shieldTaken > 0.0F) {
                    this.breakTimer = EpsShieldWall.this.breakCooldown;
                }
